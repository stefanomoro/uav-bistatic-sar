function [windowed_phase] = windowedPhase(const,radar,win_size)
%WINDOWED_PHASE compute the phase of a signal by computing window FFT
%   [windowed_phase] = windowedPhase(cont,radar,win_size)

cross_talk = radar.cross_talk.value;
%number of shifts of the window over the peak
N_cycle = floor(length(cross_talk)/win_size) * 2 - 1;
Df = zeros(N_cycle,1);
N_fft = 8*win_size;
freq_ax = (-N_fft/2:N_fft/2-1)/N_fft * 1/const.PRI;
win_idx = 1 : win_size;
...
t = (1: length(Df))*win_size/2*const.PRI;
windowed_phase = cumsum(2*pi*Df*win_size/2 * const.PRI);
windowed_phase = interp1(t,windowed_phase,(0:length(cross_talk)-1)*const.PRI,'pchip');
