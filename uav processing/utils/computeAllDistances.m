...
%% Targets distances
if(strcmp(const.setup_mode,"old"))      %old setup has classified targets
    %cars
    ...
    distance_targets = [distance_cars;distance_humans];
else
    distance_targets = zeros(size(targets,1),radar.N_PRI);
    for n = 1:radar.N_PRI
        for tgt_idx = 1:size(targets,2)
            distance_targets(tgt_idx,n) = sqrt(sum((TX.pos(:,n) - targets(:,tgt_idx)).^2))...
                + sqrt(sum( (RX.pos(:,n) - targets(:,tgt_idx)).^2));
        end
    end
end
...
scenario.distance.targets = distance_targets;
end
