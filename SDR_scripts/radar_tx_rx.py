...
"""
Radar for monostatic transmission and reception of CHIRP
"""
...
def radar_tx(usrp, tx_streamer, input_file, timer_elapsed_event, tx_statistics):
    ...
    while not timer_elapsed_event.is_set():
        try:
            num_tx_samps_now = tx_streamer.send(transmit_arr, metadata) * num_channels
...
def radar_tx_async_helper(tx_streamer, timer_elapsed_event, tx_async_statistics):
...
def main():
    ...
    usrp.set_tx_freq(uhd.libpyuhd.types.tune_request(args.freq))
